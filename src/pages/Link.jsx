...
const Link = () => {
  ...
  const { loading, data: url, error, fn } = useFetch(getUrl, { id: id, user_id: user?.id });
  ...
  return (
    <>
      ...
      <div className='flex flex-col gap-8 sm:flex-row justify-between'>
        <div className='flex flex-col items-start gap-8 rounded-lg sm:w-2/5 break-all'>
          <span className='text-6xl font-extrabold hover:underline cursor-pointer'>{url?.title}</span>
          <a href={`https://snaplnk.netlify.app/${link}`} target='_blank'
            className='text-3xl sm:text-4xl text-blue-400 font-bold hover:underline cursor-pointer'
          >
            https://snaplnk.netlify.app/{link}
          </a>
        ...
        <Card className='sm:w-3/5'>
          ...
          {stats && stats.length ?
            ...
            <CardContent>
              {
                loadingStats === false ?
                  "No Statistics Yet" : "Loading Statistics..."
              }
            </CardContent>
          }
        </Card>
